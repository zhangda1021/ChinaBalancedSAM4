...
equations       objbal, profit, dommkt, impmkt, consumption, investment;

scalar  target /1/, penalty /1e3/;

parameter	weight(i,g,r)	Weight applied on demand function adjustments;
parameter	weightf(f,i,r)	Weight applied on factor input adjustments;

weight(i,g,r) = 1;
weight(i,"gas",r)$(sameas(r,"chn")) = 1e3;

weightf(f,i,r) = 1;
weightf(f,"gas",r)$(sameas(r,"chn")) = 1e3;

objbal..        obj =e= sum((r,i)$rb(r),
      sum(f, (weightf(f,i,r)*vfm(f,i,r) * sqr(vfm_(f,i,r) /vfm(f,i,r) -1))$vfm(f,i,r)) +
...
*	Fix the investment and consumption
consumption(r)..		sum(i, vdfm_(i,"c",r)*(1+rtfd(i,"c",r)) + vifm_(i,"c",r)*(1+rtfi(i,"c",r))) =e= vom("c",r) + vom("i",r) - sum(i,vfm_("cap",i,r)) / (rbar+delta) * (gamma+delta);
investment(r)..		sum(i, vdfm_(i,"i",r)*(1+rtfd(i,"i",r)) + vifm_(i,"i",r)*(1+rtfi(i,"i",r))) =e= sum(i,vfm_("cap",i,r)) / (rbar+delta) * (gamma+delta);

*	Fix China's domestic energy production
*chnegy("gas",r)$(sameas(r,"chn"))..	vdm_("gas",r) =e= vdm("gas",r);
...
*	Fix China's energy domestic production data
*vdm_.fx("gas",r)$(sameas(r,"chn")) = vdm_.l("gas",r);
*vfm_.fx(f,"gas",r)$(sameas(r,"chn")) = vfm_.l(f,"gas",r); 
*vifm_.fx(i,"gas",r)$(sameas(r,"chn")) = vifm_.l(i,"gas",r);
*vdfm_.fx(i,"gas",r)$(sameas(r,"chn")) = vdfm_.l(i,"gas",r);
...
investment.scale(r)$round(vom("i",r)) = vom("i",r);
consumption.scale(r)$round(vom("c",r)) = vom("c",r);

*chnegy.scale("gas",r)$round(vdm("gas",r)) = vdm("gas",r);
...
execute_unload '%datadir%%ds%_%tol%.gdx', r, f, g, i, 
		...
		esubd,esubva,esubm,etrae,eta,epsilon, evd, evt, eco2;
